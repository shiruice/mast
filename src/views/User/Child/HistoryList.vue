...
<template>
  <div class="HistoryList">
    <div class="item_goods" v-for="(items, index) in goods" :key="index">
      {{ fnTime(items) }}
      <div
        class="item_goods"
        v-for="(item, index) in goodsmap[items]"
        :key="index"
      >
        <!-- 展示 -->
        <div class="item_list" v-if="type == 0">
          <div class="item_img" v-if="item.status == true">
          ...
          <div class="item_img" v-if="item.status == false">
            <span class="goods_img imgbox">
              <van-image
                :src="apiHost + '/image' + item.mainPictureUrl"
                fit="contain"
                class="img_boy"
          ...
          <div class="item_text">
            ...
            <div class="item_button" v-if="item.status == true">
        ...
        <div v-else :class="index + 1 == goods.length ? 'item_listlsat ' : ''">
          ...
          >
            <div
              :class="index + 1 == goods.length ? ' item_list' : 'item_list'"
            >
              <div class="item_img">
                <span class="goods_img" @click="goDetail(item.skuId)">
...
export default {
  ...
  data() {
    return {
      ...
      apiHost: this.$auth.os.lxapp
        ...
        : location.origin,
      url: {
        delete: `/api/shoppe-api-service/mobile/sku/browse-history-delete`,
  ...
  methods: {
    ...
    goDetail(value) {
      this.$router.push({
        name: "ProductDetails",
        query: { skuId: value, type: 6 },
      });
    },
    DeteleArr() {
      // console.log(this.$parent);
      if (this.selectedData.length > 0) {
        deleteFunction(this.url.delete, this.selectedData).then((res) => {
          if (res.code == "000000") {
            Toast.success("删除成功");
            this.$emit("callback");
          }
...
.imgbox:after {
  ...
  width: 100%;
  height: 100%;
  background-image: url("../../../assets/img/imgafler.png");
