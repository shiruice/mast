...
export default {
  ...
  data() {
    return {
      ...
      api: {
        management: `/api/shoppe-api-service/moblie/invoice/management/`,
      },
    };
  },
  components: {
  ...
  methods: {
    ...
    onSubmit() {
      ...
      if (this.radio == 1) {
        ...
        data.invoiceTitle = this.list.invoiceTitle;
        data.socialCreditCode = this.list.socialCreditCode;
      }
      console.log(data);
      postFunction(this.api.management, data).then((res) => {
        if (res.code == "000000") {
          this.$router.push({
            name: "InvoiceTitle",
