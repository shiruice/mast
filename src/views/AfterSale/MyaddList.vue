...
export default {
  ...
  methods: {
    renturnconfirm(val) {
      let address = {
        address: val.address,
        contacts: val.recipientName,
        contactsAddressCityId: val.cityId,
        contactsAddressLine: val.addressLine,
        contactsAddressProvinceId: val.provinceId,
        contactsCountyId: val.countyId,
        contactsPhone: val.recipientPhone,
        postCode: val.postalCode,
      };
      if (this.$route.query.getOrSend) {
        this.$store.commit("SET_SENDADDRESS", address);
      } else {
        this.$store.commit("SET_GETADDRESS", address);
      }
      this.$router.push({
        name: "returnRequest",
        query: {
